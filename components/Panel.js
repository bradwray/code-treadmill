...
export default function Panel() {
  ...
  let DisplaysContainer = store.leftAligned
    ? DisplaysContainerLeft
    : DisplaysContainerTop;

  React.useEffect(() => {}, []);

  const handleWannaRace = () => {
    setStore({ ...store, workout: "none" });
    router.push("/race");
  };
  ...
  return (
    <Surface>
      <ControlsContainer>
        <FlipButton
          onClick={() =>
            setStore({ ...store, leftAligned: !store.leftAligned })
          }
        >
          {!store.leftAligned ? `⤵` : `⤴`}
        </FlipButton>
        ...
        <JoinBtn onClick={() => handleWannaRace()}>Wanna Race?</JoinBtn>
      </ControlsContainer>
      <DisplaysContainer>
        <ResultsDisplay />
        <ProgressChart />
